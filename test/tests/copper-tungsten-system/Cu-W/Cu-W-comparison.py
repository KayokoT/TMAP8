...
"""
This file creates:
    - conservation of mass plots for each Cu_W_run**
...
legend = ["no traps", "traps"]  # CHANGE THIS TO WHAT EACH RUN REPRESENTS
...
def plot_penalty_check(case, run_id, var_key, gold_key, interface_label, filename_prefix):
    ...
    Args:
        ...
        run_id (str): identifies this run (e.g. "Cu_W_run01_main") -- used in title/filename
        ...
        interface_label (str): e.g. "Cu/W" -- used in title
        filename_prefix (str): e.g. "Cu_W-penalty"
    """
...
main_csvs = sorted(
    glob.glob("Cu_W_run[0-9][0-9]_main.csv"),
...
for m in main_csvs:
    ...
    time, conc_Cu, conc_W, gold_sol, var_sol, right_outflux, mass_from_con, mass_from_flux = read_csv_from_TMAP8(
        m,
        ["time", "concentration_at_x_Cu", "concentration_at_x_W", "gold_solubility_ratio",
...
dist_csvs = glob.glob("Cu_W_run*_*_*.csv")
dist_pattern = re.compile(r"Cu_W_(run\d+)_(Cu|W)_\d+\.csv$")
...
ax.minorticks_on()
plt.savefig("Cu_W-copper-vs-time.png", bbox_inches="tight", dpi=300)
plt.close(fig)
...
plt.grid(visible=True, which="major", color="0.65", linestyle="--", alpha=0.3)
ax.minorticks_on()
plt.savefig("Cu_W-tungsten-vs-time.png", bbox_inches="tight", dpi=300)
plt.close(fig)

# ============ Concentration as a function of distance ============
# region boundaries (Cu first, then W)
x_CuW = Cu_thickness
x_W = Cu_thickness + W_thickness

regions = {
    "Cu": lambda x: (x >= 0) & (x < x_CuW),
    "W": lambda x: (x >= x_CuW) & (x <= x_W),
}

shading = [
    ("Copper", 0, x_CuW, "orange"),
    ("Tungsten", x_CuW, x_W, "gray"),
]
...
for i, run in enumerate(runs_x):
        ...
        # Cu first, then W
        x_full = np.concatenate([x_Cu_, x_W_])
        c_full = np.concatenate([c_Cu_, c_W_])
...
ax.legend(loc="best")
plt.savefig("Cu_W-vs-dist.png", bbox_inches="tight", dpi=300)
plt.close(fig)
...
ax.legend(loc="best")
ax.set_title("H Concentration over Distance", fontsize=15)
plt.savefig("Cu_W-copper-only-vs-dist.png", bbox_inches="tight", dpi=300)
plt.close(fig)
...
format_ax(ax, (x_CuW, x_W))
ax.legend(loc="best")
ax.set_title("H Concentration over Distance", fontsize=15)
plt.savefig("Cu_W-tungsten-only-vs-dist.png", bbox_inches="tight", dpi=300)
plt.close(fig)
...
# assumes run01 is the no trapping case, particularly to compare against other permeation rates to calculate RMSPE
fig = plt.figure(figsize=[6.5, 5.5])
gs = gridspec.GridSpec(1, 1)
ax = fig.add_subplot(gs[0])
...
ax.minorticks_on()
plt.savefig("Cu_W-permeation-rate.png", bbox_inches="tight", dpi=300)
plt.close(fig)
