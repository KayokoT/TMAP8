...
RUN_ID = run01 #default, change this when running by adding RUN_ID=run0*
...
[Materials]
  ...
  [flux_recombination_surface]
    ...
    expression = '-2 * ${Cu_kr} * C_M_Cu ^ 2' #at/mum^2/s
  []
[]
...
[InterfaceKernels]
  # Penalized continuity Cuith solubility jump at W/Cu interface
  [W-Cu-interface]
...
[Postprocessors]
  ...
  [flux_difference]
    type = ParsedPostprocessor
    expression = '-left_influx - right_outflux' # negative sign on influx to account for outCuard normal vector direction
